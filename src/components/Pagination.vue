<template>
    <div class="pagination-container">
        ...
        <div class="pagination-center">
            ...
            <span class="page-size" @click="changePage(currentPage + 1)" v-show="currentPage < pagerCount - 1">{{ currentPage + 1 }}</span>
            <span class="page-size next" @click="changePage(currentPage + 2)" v-show="currentPage < pagerCount - 2">{{ currentPage + 2 }}</span>
            <span class="iconfont icon-more-outline" v-show="pagerCount - currentPage > 3" ></span>
            <span class="page-size" @click="changePage(pagerCount)" v-show="currentPage < pagerCount">{{ pagerCount }}</span>
        </div>
        <div class="turn-page-box"><i class="iconfont icon-right" @click="checkNext" v-show="currentPage < pagerCount"></i></div>
    </div>
</template>
